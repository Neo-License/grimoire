Feature: Project initialization
  ...
  Scenario: Init prints a two-step onboarding flow for existing projects
    Given the project has existing source code
    When I run "grimoire init"
    Then the next-steps output should suggest running "/grimoire:discover" as the next step
    And it should not suggest running "grimoire map" before discover
    And it should note that codebase-memory-mcp must be installed first

  Scenario: Init prints a one-step onboarding flow for greenfield projects
    Given the project directory is empty or contains only config files
    When I run "grimoire init"
    Then the next-steps output should suggest starting with "/grimoire:draft"
    And it should not suggest running discover or audit
    And it should not show the codebase-memory-mcp install note
